...
if(($row['Level'] == 1) && ($row['Disable'] == TRUE)){
	...
	header("Location:http://10.16.1.102:8082/branders/admin.php?id=".$id);						
							}
							else

							{
								$auto = generate();
								if($row['Disable'] == TRUE) {
								$id = $row['EmpID'];  $id2 = $row['Manager']; header("Location:http://10.16.1.102:8082/branders/QA.php?id=".$id."&x=".$auto);
								//echo generate();
